...
FROM cidade, estado
WHERE cidade.ativo = 'S' AND cidade.estado_id = estado.id;
...
SELECT cliente.nome, cidade.nome
FROM cliente, cidade
WHERE cidade.nome = 'PARANAVAÍ' AND cliente.cidade_id = cidade.id;
...
-- 7.	Escreva o comando que liste os nomes dos funcionários e a sigla do respectivo estado. 
SELECT cliente.nome, estado.sigla
FROM cliente, cidade, estado
WHERE cliente.cidade_id = cidade.id AND cidade.estado_id = estado.id;
